-- Config RPCs: write activity_log rows (no stamp columns on config tables).
...
-- ── Tags ─────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.create_tag(p_workstation_id uuid, p_data jsonb)
 RETURNS jsonb LANGUAGE plpgsql SET search_path TO 'public'
AS $function$
...
BEGIN
  ...
  RETURNING * INTO v_row;
  PERFORM log_activity(p_workstation_id, 'tag', v_row.id, v_row.name, 'created');
  RETURN row_to_json(v_row)::jsonb;
END;
$function$;
...
CREATE OR REPLACE FUNCTION public.create_note_folder(p_workstation_id uuid, p_name text)
 RETURNS jsonb LANGUAGE plpgsql SET search_path TO 'public'
AS $function$
DECLARE v_row note_folders%rowtype; v_existed boolean := false;
BEGIN
  IF NOT EXISTS(SELECT 1 FROM workstation_members WHERE workstation_id = p_workstation_id AND user_id = auth.uid())
  THEN RAISE EXCEPTION 'access_denied'; END IF;
  INSERT INTO note_folders(workstation_id, name) VALUES (p_workstation_id, p_name)
  ON CONFLICT (workstation_id, name) DO NOTHING
  RETURNING * INTO v_row;
  IF v_row.id IS NULL THEN
    v_existed := true;
    SELECT * INTO v_row FROM note_folders WHERE workstation_id = p_workstation_id AND name = p_name;
  END IF;
  IF NOT v_existed THEN
    PERFORM log_activity(p_workstation_id, 'note_folder', v_row.id, v_row.name, 'created');
  END IF;
  RETURN row_to_json(v_row)::jsonb;
END;
$function$;
...
BEGIN
  ...
  RETURNING * INTO v_row;
  IF NOT FOUND THEN RAISE EXCEPTION 'not_found'; END IF;
  PERFORM log_activity(v_row.workstation_id, 'note_folder', v_row.id, v_row.name, 'updated');
  RETURN row_to_json(v_row)::jsonb;
END;
$function$;
...
CREATE OR REPLACE FUNCTION public.delete_note_folder(p_folder_id uuid, p_workstation_id uuid)
 RETURNS void LANGUAGE plpgsql SET search_path TO 'public'
AS $function$
DECLARE v_other_id uuid; v_label text;
BEGIN
  IF NOT EXISTS(SELECT 1 FROM workstation_members WHERE workstation_id = p_workstation_id AND user_id = auth.uid())
  THEN RAISE EXCEPTION 'access_denied'; END IF;
  ...
  IF p_folder_id = v_other_id THEN RAISE EXCEPTION 'cannot_delete_other'; END IF;
  SELECT name INTO v_label FROM note_folders WHERE id = p_folder_id;
  UPDATE notes SET folder_id = v_other_id WHERE folder_id = p_folder_id;
  DELETE FROM note_folders WHERE id = p_folder_id;
  PERFORM log_activity(p_workstation_id, 'note_folder', p_folder_id, v_label, 'deleted');
END;
$function$;
